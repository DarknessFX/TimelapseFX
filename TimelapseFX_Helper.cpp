...
void CaptureWindow(HWND hwndTarget, std::string sFile)
{
    // Init COM
    ...
    if (auto lerr = fopen_s(&lfile, sFile.c_str(), "wb"); lerr != 0)
        return;
...
void CaptureFromDLL(HWND hTargetWnd, std::string sFile)
{
    CaptureWindow(hTargetWnd, sFile);
}
#pragma endregion